...
import {
  ...
  HUNTER_LEVEL_CAP as DATA_HUNTER_LEVEL_CAP,
...
// Hunter level cap (dungeon gate, canonical in ./data/tuning): gainExp
// banks nothing at/above this.
export const HUNTER_LEVEL_CAP = DATA_HUNTER_LEVEL_CAP;
...
export class GameSimulation {
  ...
  public whirlSlow: Map<string, { burst: number; lastZoneId: string | null }> = new Map(); // Whirl slow (public: HunterBrain mirrors spin state here)
  ...
  // Hunter brain: autonomous decision-making (targeting, needs, parties,
  // dungeon routing, state machine). Decides; this class acts + owns state.
  public brain: HunterBrain = new HunterBrain(this);
  ...
  public update(dtSeconds: number) {
    ...
    if (!this.agentConfig.partiesEnabled) {
      ...
      for (const h of this.hunters) {
        if (h.state === 'LOOKING_FOR_PARTY') this.brain.leaveForHunt(h);
        if (h.state === 'DUNGEON_LOBBY') {
          this.brain.releaseLobbySeat(h);
          this.brain.leaveForHunt(h);
        }
      }
      this.partyTimer = 0;
    } else {
      ...
      if (this.partyTimer >= 3) {
        this.partyTimer = 0;
        this.brain.runPartyFormation();
      }
    }

    // 3. Update Hunters (AI & Autonomous Decision Making — see ./brain.ts)
    for (let i = this.hunters.length - 1; i >= 0; i--) {
      this.brain.updateHunterAI(this.hunters[i], effectiveDt);
    }

    // 3a. Portal lobby: a full house of seated hunters descends.
    this.brain.tickLobbyTeleport();
  ...
  // --------------------------------------------------------------------------
  // --------------------------------------------------------------------------
  // 4. HUNTER AUTONOMOUS AI ENGINE — see ./brain.ts (HunterBrain).
  // updateHunterAI + targeting + needs + parties + dungeon routing live
  // there; this class owns world state and acts on brain orders.
  // --------------------------------------------------------------------------
  ...
  public resolveHunterCombat(hunter: Hunter, monster: Monster, dt: number) {
   ...
   */
  public zoneOf(gx: number, gy: number): 0 | 1 | 2 | 3 | 4 {
  ...
  // When hunter arrives at a designated building (public: brain routes arrivals here)
  public executeBuildingVisit(hunter: Hunter, building: Building) {
   ...
   * loop straight back.
   */
  // Completing a building visit (public: brain service states complete here)
  public finishStoreInteraction(hunter: Hunter) {    const completedState = hunter.state;
    ...
    this.brain.evaluateTownNeeds(hunter);
  }
  ...
  public canAffordForgeUpgrade(hunter: Hunter): boolean {
  ...
  public moveTowards(hunter: Hunter, targetX: number, targetY: number, speed: number): boolean {
